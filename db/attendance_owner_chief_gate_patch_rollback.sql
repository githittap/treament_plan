...
do $$ declare p oid:='public.review_manual_attendance(bigint,text)'::regprocedure; task7_current jsonb; begin
 ...
 task7_current:=jsonb_set(task7_current,'{canonical,schema,contents}',coalesce((select jsonb_agg(x order by x->>'name') from jsonb_array_elements(task7_current#>'{canonical,schema,contents}') x where x->>'name' not in ('attendance_owner_chief_gate_patch_marker','attendance_owner_chief_gate_patch_marker_pkey')),'[]'::jsonb));
 if not exists(select 1 from employee_hub_private.push_subscriptions_migration_marker m where m.fingerprint='task7-push-v3-5d9a' and m.manifest='push-v3-private-5d9a' and m.canonical=(task7_current->'canonical') and m.identity=(task7_current->'identity') and md5(m.canonical::text)='8f6afbfff63ecf0dac24d41f76b42837' and md5((task7_current->'canonical')::text)='8f6afbfff63ecf0dac24d41f76b42837') then raise exception 'Task 7 marker canonical or identity mismatch; preserve state and stop rollback'; end if;
