...
select public._install_employee_hub_access_gate(to_regclass('public.confidential_access'));
select public._install_employee_hub_access_gate(to_regclass('public.confidential_records'));
...
revoke all on function public.disable_employee_account_preserve_records(uuid,text,text,date) from anon;
revoke all on function public.approve_employee_profile(uuid) from public;
revoke all on function public.approve_employee_profile(uuid) from anon;
revoke all on function public.update_employee_profile_field(uuid,text,text) from public;
revoke all on function public.update_employee_profile_field(uuid,text,text) from anon;
revoke all on function public.revoke_employee_profile_approval(uuid) from public;
revoke all on function public.revoke_employee_profile_approval(uuid) from anon;
