...
with updated_fields as (
  ...
  from (
    ...
    where t.id = 1
      and field->>'key' not in ('근무요일', '시업', '종업', '휴게')
    union all
    ...
    union all
    select jsonb_build_object('key','공휴일근무','type','select','label','공휴일 근무','options',jsonb_build_array('해당 없음','포함'),'default','해당 없음'), 1001
  ) f
)
update public.doc_templates t
set fields = updated_fields.fields,
    presets = jsonb_set(t.presets, '{부원장}', coalesce(t.presets->'부원장','{}'::jsonb) - '근무요일', true),
    body_html = $doc$
...
<p>병원명 <b>아산정플란트치과의원</b> · 대표자 <b>정용태</b><br>직원 성명 <b>{{성명}}</b> · 생년월일 <b>{{생년월일}}</b><br>주소 <b>{{주소}}</b></p>
...
<p>1. 직원의 근로시간 및 휴게시간은 다음과 같다.<br>{{근무시간표}}<br>{{공휴일근무문구}}2. 구체적인 요일·시간은 위 시간표 및 별도 스케줄표에 따르며, 변경이 필요한 경우 관계 법령과 근로계약 절차에 따라 서면 또는 전자문서로 명시한다.<br>3. 연장·휴일·야간근로는 사전 승인 절차로 관리하며, 실제 근로시간과 수당은 관계 법령에 따라 처리한다.</p>
<h3 style="color:#156f72;border-bottom:2px solid #156f72;padding-bottom:3px">제5조 (임금)</h3>
<p>1. 임금 유형 <b>{{임금유형}}</b> · 금액 <b>{{임금액}}</b><br>□ 상기 금액에는 <b>{{포괄내역}}</b>이 포괄되어 있다.<br>2. 임금은 매월 1일~말일을 산정기간으로 하여 <b>{{지급일}}</b>에 직원 본인 계좌로 입금하며, 지급일이 휴일이면 전일 또는 다음 영업일에 지급한다.<br>3. 의원은 세전 임금제를 운영하며, 노동법·세법에 맞게 제세공과금을 공제 후 지급한다.<br>4. 은혜적·호의적 인센티브는 부정기적 금품으로 통상임금 및 퇴직금에 포함되지 아니한다.<br>5. 지각·조퇴·결근 등 근무하지 못한 시간은 무급 반영되며, 중도 입·퇴사는 일할계산한다.</p>
...
<p>1. 1년 이상 근무 시 근로자퇴직급여보장법에 따라 퇴직급여(퇴직연금 등)를 지급한다.<br>2. 사직하고자 하는 경우 원활한 인수인계를 위하여 3주 전 사직서를 제출하도록 협조한다. 퇴직 절차 및 손해배상 여부는 관계 법령에 따른다.</p>
...
<p>다음 각 호의 사유가 발생한 경우에도, 의원은 관계 법령과 취업규칙상 절차 및 정당한 사유 요건을 충족하는 범위에서 근로계약의 종료 또는 징계 여부를 판단한다.<br>1. 정당한 사유 없이 업무지시에 불복 2. 고의·과실로 경제적 손해 또는 명예·신뢰에 악영향 3. 불성실 근무·근무질서 문란 4. 무단결근 3회 이상(연락두절 시 자진퇴사 간주) 5. 직장 내 성희롱·괴롭힘 6. 환자 불친절·업무미숙 등 민원으로 근무가 어렵다고 판단 7. 의료관계법령 위반 또는 자격 상실 8. 시말서 1개월 3회 이상 제출 9. 기타 이에 준하는 사유로 근로계약관계 지속이 어려운 경우</p>
<h3 style="color:#156f72;border-bottom:2px solid #156f72;padding-bottom:3px">제11조 (기타)</h3>
<p>1. 본 계약서에 명시되지 않은 사항은 근로관계법령 및 당사 규정에 따른다.<br>2. 퇴사 시 병원에서 받은 물품은 원상복귀 후 반납한다. 미반납 물품의 처리 및 금전 정산은 관계 법령에 따른다.<br>3. 퇴직 시 임금·퇴직금 등 금품은 관계 법령에서 정한 기한과 절차에 따라 지급한다.</p>
