...
do $$ declare p record; role_list text; begin for p in select * from public.notice_attachments_migration_policy_snapshot order by schemaname,tablename,policyname loop role_list:=case when p.roles is null or cardinality(p.roles)=0 then 'PUBLIC' else array_to_string(p.roles,', ') end; execute format('create policy %I on %I.%I as %s for %s to %s%s%s',p.policyname,p.schemaname,p.tablename,p.permissive,p.cmd,role_list,case when p.qual is null then '' else ' using ('||p.qual||')' end,case when p.with_check is null then '' else ' with check ('||p.with_check||')' end); end loop; end $$;
