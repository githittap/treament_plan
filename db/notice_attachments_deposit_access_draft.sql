...
do $$ begin if exists (select 1 from information_schema.tables where table_schema='public' and table_name='notice_attachments_migration_snapshot') then raise exception 'already applied; preserve snapshot and stop migration'; end if; end $$;
begin;
...
insert into public.notice_attachments_migration_rls_snapshot(schemaname,tablename,rls_enabled) select n.nspname,c.relname,c.relrowsecurity from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname in ('notices','deposits') on conflict do nothing;
