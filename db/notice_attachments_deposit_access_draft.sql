...
create or replace function public.guard_notice_immutable_fields() returns trigger language plpgsql security definer set search_path=public,pg_temp as $$ begin if tg_op='INSERT' then if new.author_id is distinct from auth.uid() then raise exception 'notice author must match authenticated user'; end if; select p.name into new.author from public.profiles p where p.user_id=auth.uid(); if new.author is null then raise exception 'notice author profile missing'; end if; return new; end if; if new.author_id is distinct from old.author_id or new.author is distinct from old.author or new.created_at is distinct from old.created_at then raise exception 'notice author and created_at are immutable'; end if; return new; end; $$;
