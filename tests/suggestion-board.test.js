...
test('게시글 수·좋아요 수·사람별 집계를 계산한다', () => {
  ...
  const result = context.summarizeSuggestions([
    { id: 1, user_id: 'u1' }, { id: 2, user_id: 'u1' }, { id: 3, user_id: 'u2' }
  ], [{ suggestion_id: 1 }, { suggestion_id: 1 }, { suggestion_id: 2 }]);
...
test('자기 글·종료 캠페인·이미 좋아요에는 좋아요를 허용하지 않는다', () => {
  const context = loadContext();
  assert.equal(context.canLikeSuggestion({ user_id: 'u1' }, 'u1', campaign(), false), false);
  assert.equal(context.canLikeSuggestion({ user_id: 'u2' }, 'u1', campaign({ ends_at: '2026-08-31' }), false), false);
  assert.equal(context.canLikeSuggestion({ user_id: 'u2' }, 'u1', campaign(), true), false);
  assert.equal(context.canLikeSuggestion({ user_id: 'u2' }, 'u1', campaign(), false), true);
});
...
test('종료 전에는 수상을 숨기고 종료 후에는 최소 공개 필드만 반환한다', () => {
  ...
  assert.deepEqual(JSON.parse(JSON.stringify(context.winnerVisibility(campaign({ ends_at: '2026-09-30' }), rows, rows))), []);
  assert.deepEqual(JSON.parse(JSON.stringify(context.winnerVisibility(campaign({ ends_at: '2026-08-31' }), rows, rows))), rows);
});
