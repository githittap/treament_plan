...
try{
  ...
  await q('reset role');await q('delete from public.manual_attendance_status_history');await q('delete from public.attendance_manual_revisions');await q('delete from public.attendance_manual_entries');await q('set role authenticated');await q(`select set_config('app.test_uid','${staff}',false)`);
  const zero=await rpc('submit_manual_attendance_v2',`'2026-09-25','09:00','18:00',0,0,'',0,'',0,null,false`);assert.equal(zero[0].overtime_min,0);assert.equal((await q('select count(*)::int n from public.attendance_manual_revisions where payload ? \'lunch_overtime_raw_text\''))[0].n,1);
  await q('reset role');let zeroRollback='';try{await db.exec(fs.readFileSync('db/attendance_manual_v2_rollback.sql','utf8'))}catch(e){zeroRollback=String(e)}assert.match(zeroRollback,/rollback stopped/);assert.equal((await q('select count(*)::int n from public.attendance_manual_entries'))[0].n,1);
