...
const owner='11111111-1111-1111-1111-111111111111',staff='22222222-2222-2222-2222-222222222222',chief='33333333-3333-3333-3333-333333333333',ownerTwo='44444444-4444-4444-4444-444444444444';
try{
  ...
  await q('reset role');await q(`insert into public.profiles values('${ownerTwo}','보조원장','owner',true,true)`);await q('set role authenticated');
  let self='',lastOwner='',directDelete='';try{await q(`select public.set_employment_status('${owner}','자진퇴사','2026-09-23','self')`)}catch(e){self=String(e);await q('rollback')}await q('reset role');await q(`update public.profiles set active=false where user_id='${ownerTwo}'`);await q('set role authenticated');try{await q(`select public.set_employment_status('${owner}','자진퇴사','2026-09-23','last')`)}catch(e){lastOwner=String(e);await q('rollback')}try{await q(`delete from public.profiles where user_id='${staff}'`)}catch(e){directDelete=String(e);await q('rollback')}assert.match(self,/cannot change your own employment status/);assert.match(lastOwner,/cannot change last active owner/);assert.match(directDelete,/permission denied|row-level security/);
