...
`;
try {
  await db.exec(prelude + draft); assert.deepEqual((await query("select id,name,public from storage.buckets where id='leave-docs'"))[0],{id:'leave-docs',name:'leave-docs',public:false}); const staff='11111111-1111-1111-1111-111111111111',chief='33333333-3333-3333-3333-333333333333',manager='22222222-2222-2222-2222-222222222222';
  ...
  await query(`insert into storage.objects(bucket_id,name,owner_id) values ('leave-docs','${staff}/tmp/orphan.pdf','${staff}'),('leave-docs','${staff}/tmp/kept.pdf','${staff}')`); await query(`insert into public.leave_application_documents(request_id,user_id,original_name,storage_path,mime_type,size_bytes) values (1,'${staff}','kept.pdf','${staff}/tmp/kept.pdf','application/pdf',100)`); let duplicate=''; try { await query(`insert into public.leave_application_documents(request_id,user_id,original_name,storage_path,mime_type,size_bytes) values (1,'${staff}','duplicate.pdf','${staff}/tmp/kept.pdf','application/pdf',100)`); } catch(error) { duplicate=String(error); } assert.match(duplicate,/duplicate key/); assert.equal((await query('select count(*)::int n from public.leave_application_documents'))[0].n,1);
