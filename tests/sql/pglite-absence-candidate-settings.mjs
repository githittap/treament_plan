...
try{
  ...
  await db.exec(fs.readFileSync('db/absence_candidate_settings.sql','utf8'));
  assert.equal((await q("select value from public.app_settings where key='absence_confirm_after_minutes'"))[0].value,'12','ON CONFLICT must preserve the owner value');
  await assert.rejects(db.exec(fs.readFileSync('db/absence_candidate_settings_rollback.sql','utf8')),/user values/);
  ...
  assert.equal((await q("select count(*)::int n from public.app_settings where key like 'absence_%'"))[0].n,0);
  console.log('PGLITE_ABSENCE_CANDIDATE_SETTINGS_PASS: seed, owner-only RLS, default-only rollback, user-value stop');
