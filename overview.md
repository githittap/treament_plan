...
- 결과보고서 `Z:\11_codex\00_결과보고서\직원허브_구현결과보고서_2026-09-22.html`(SHA256 `1F0A93119B9E2B5E1A96D9B6CE6D57DDDAEAF631720C3F71A1106CBC50FEF428`)과 사용설명서 `Z:\11_codex\03_병원운영·전산\직원허브_수정지침_시안\직원허브_사용설명서.docx`(SHA256 `15F3CC405FD45A5EFA306BBC5134F2589FCD71020287F22D186A72A0664328B0`)를 최신화했다. HTML 자리표시자 제거·하단 저장 스크립트 동일성, DOCX 통합 문의함 절·8개 안내·기존 이미지 보존을 정적으로 확인했다. Pandoc→Edge headless→PDF→6페이지 PNG 시각 QA에서 번호 결함 발견·수정, 전 페이지 가독성/잘림 확인 PASS(Word 원본 렌더가 아닌 대체 렌더)다.
...
- `직원허브 5차.zip` SHA256 `A177836684189BFAA4D0733F1EFF8D39B58B9F5FE71791E1F551DCD7C0EC5E3A`의 요구 대조를 완료했고, 7개 원본은 보존 복사본과 이름별 SHA256 일치·Downloads 원본 보존을 확인했다.
- 근무표·수기근태·결근/미기록 후보·재직상태/접속차단은 구현·검증·운영 반영까지 완료됐다.
- 문자 그대로 남은 항목은 실제 Auth 계정 영구삭제 하나뿐이며, 현재는 기록보존형 영구 접속차단으로 안전하게 대체 중이다. 다음 활성 범위는 `상담문의 등 일원화.zip`이다.
...
- 운영 후검증 보완: snapshot marker는 RLS만으로 충분하지 않아 `PUBLIC`·`anon`·`authenticated`의 table privilege를 모두 회수했다. 이미 적용된 환경은 `db/absence_candidate_settings_acl_hardening.sql`이 marker 행·설정값을 바꾸지 않고 같은 ACL만 멱등 보완한다.
- 운영 migration `employee_hub_absence_candidate_settings_20260922`와 ACL hardening `employee_hub_absence_candidate_settings_acl_hardening_20260922`는 성공했다. keys=`0/true`, snapshot=2, PUBLIC·anon·authenticated privilege=false, policy=0을 postflight로 확인했다. `main` commit `3efbff9` push, Pages run `35693355918` success, 공개 `hr.html?v=3efbff9` HTTP 200(509621 bytes) 및 CandidateText·ConfirmDelay·ManualExclude·LoadGuard 표식=true다.
...
- 최초 `직원허브 수정 지침.zip`에서 외부자료 없이 가능한 승인 범위 구현·운영적용·배포·문서화는 종료했다. 다음 활성 범위가 `직원허브 5차.zip` 원본 보존·해시·인벤토리·요구 대조라는 과거 기록은 위 최신 상태로 대체됨이며, `상담문의 등 일원화.zip`이 다음 활성 범위다.
...
- 다음 순서가 Task10 역할별 단계배포·최종인수 → Task11 설명서이고 `직원허브 5차.zip`이 미열람이라는 과거 기록은 최신 상단 상태로 대체됨이다. `상담문의 등 일원화`는 다음 활성 후속이다. 실제 직원 Push는 계속 금지한다.
...
- 최종 상태가 Sol High 7차 문서 대조 대기이고 Task7/Task8 production 미적용이라는 과거 기록은 최신 상단 상태로 대체됨이다. 실제 직원 메일·Push·생체입력, 자격증명 입력, 원본 삭제, 대량 이관, 보안 완화, 새 비용은 금지한다. `상담문의 등 일원화.zip`은 다음 활성 후속이다.
...
- Sol 독립 재검증과 `직원허브 5차.zip` 열람 금지라는 과거 대기 기록은 최신 상단 상태로 대체됨이다.
...
운영 전 Storage 검증과 `직원허브 5차.zip` 미열람 대기라는 과거 기록은 최신 상단 상태로 대체됨이다. K3 관련 기록은 당시 상태로 보존한다.
