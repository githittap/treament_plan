...
- ⏳ **외부 상담 연결 — 네이버 톡톡 챗봇 API 신청(원장)** — 원장은 파트너센터 접속법을 안다(URL 불필요). 할 일: 톡톡 파트너센터에서 챗봇 API 사용 신청(심사 1~2일) → 승인되면 Claude가 드리는 받는 주소(웹훅 URL)를 붙여 넣고 발급 키를 금고에 한 줄. 받는 쪽 코드(`navertalk-webhook`)는 준비 중. 카카오는 지금처럼 직원 응답(연결 안 함), 당근은 공식 연동 없음.
...
- ⏳ **jung-plant.com·직원허브 메뉴 정리 — 조직도 화면에서 끌어 놓기(원장)** — 지금 메뉴 구조를 뽑는 중이며, 원장이 끌어다 옮길 수 있는 조직도 화면을 만들어 링크를 드린다. 원장이 옮긴 결과는 자동 저장되어 Claude가 읽고 구현한다.
