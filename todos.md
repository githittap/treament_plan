...
- ⏳ **`#뻐스디`만 치면 장부에 일이 여러 개인데 어떻게 고르나? — 원장 질문·판단 필요 (2026-09-23)**: 원장 원문 *"어떤걸 해야하는지 명시 안하면 BUSD에 여러개 있을텐데 어떻게 하는가"*. **지금 규칙의 빈틈이다** — `#뻐스디`만 치면 목록을 보여 주기만 하고 고르는 방법이 없다. 폴더 없이 열면 열린 일이 12건 넘게 나온다. **추천안**: ①`#뻐스디` → 열린 일을 **번호 붙여** 급한 것부터 7개(나머지는 "그 외 n건") ②`#뻐스디 3` → 3번을 이어서 한다 ③`#뻐스디 직원허브` → 제목에 그 말이 든 것만 추리고, **하나뿐이면 바로 이어간다**. 고르면 AI 가 그 일의 taskId 로 상태를 `진행`·담당 AI 로 바꾼 뒤 시작한다. `ㅇㅋ` 주시면 `BUS-REGISTER.md` 에 넣고 코덱스에도 같이 반영한다(코코).
