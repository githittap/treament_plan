...
- ⏳ **BUSD 한 줄(`BUSD 읽고 이어서 해줘`)을 어디까지 등록할까? — 원장 판단 (2026-09-23)**: 원장 요구 *"어떤 ai에서 별도폴더없이 버스디의 뭐뭐를 봐라 하고 작업을 넘나들면서 하고싶다 — 코덱스에서는 뭐라고 키미에서는 뭐라고 하면되냐고"*. 답은 **어디서나 `BUSD 읽고 이어서 해줘` 한 줄**. 다만 각 AI 지침에 "그 말을 들으면 BUSD 를 읽어라"가 등록돼야 실제로 먹힌다. **클로드 쪽은 이 세션이 바로 할 수 있고, 코덱스·키미는 「T8 직원허브 · 검증채점」 세션의 ②번 작업과 겹친다.** → ①이 세션이 클로드만 맡고 나머지는 그쪽에 남길까 ②이 세션이 셋 다 할까? 한 마디만 주시면 된다.
