...
- 🔴 **살아 있는 웹훅 토큰이 공개 저장소에 평문으로 있다 — 원장 판단 필요 (2026-09-23 발견)**: `todos.md` 의 MacroDroid 안내 줄(약 433~434행)에 `ai-billing-webhook` 의 **주소와 인증 토큰(32자)이 같이** 적혀 있다. 2026-09-14 커밋 `1dde6cd`부터 공개돼 있었다(이번 세션이 넣은 것 아님). **지금도 유효한 키다** — 값을 화면에 내지 않고 SHA256 만 대조해 `webhook_secrets` 의 `ai_billing_webhook` 행과 **정확히 일치**함을 확인했다. 즉 주소와 토큰을 본 사람은 누구나 AI 비용 기록을 우리 시스템에 써 넣을 수 있다.<br>⚠️ **Claude 는 손대지 않았다**(전역 규칙 「보안·공개 판단은 원장이 한다」). 원장 지시가 있어야 움직인다.<br>👉 **줄을 지우는 것만으로는 안 된다** — git 이력에 9/14부터 남아 있어 이미 본 사람은 계속 쓸 수 있다. 실제 해결은 **토큰 교체**다: 새 값을 만들어 `webhook_secrets` 의 `ai_billing_webhook` 행을 바꾸고, 폰 MacroDroid 매크로의 헤더 값도 같이 바꾸면 끝(2분). **"바꿔라" 한 마디면 진행한다.**<br>ℹ️ 나머지 2개(`ai_usage_sync_sha256`·`navertalk_webhook_sha256`)는 이름대로 해시로 저장돼 있어 같은 문제가 없다.
- ⏳ **폰에서 알림 켜기 — 원장님이 직접 (2026-09-23)**: 푸시는 운영에서 살아 있고 1분마다 돈다(응답 200 확인). 남은 건 원장님 폰에서 한 번 누르는 것뿐이다. 경로: `https://jung-plant.com/hr.html` → 로그인 → 위쪽 **🩺 환자관리·진료** → 아래줄 **📚 업무자료** → 화면 맨 아래 **🔔 모바일 알림** → **[이 기기에서 알림 받기]** → "허용". ⚠️ 아이폰은 **홈 화면에 추가한 뒤 그 아이콘으로** 열어야 된다(Safari 탭에서는 안 됨, iOS 16.4+). 갤럭시는 Chrome 탭에서 바로 되고 시크릿만 안 된다. 누른 뒤 **연차 한 건 신청**해서 실장·원장 폰에 알림이 오는지 확인하면 끝. 직원 공지(`Z:\09_claude-output\03_병원운영·전산\직원허브_앱설치_공지_2026-09-09.html`)에도 같은 5단계를 넣어 두었다.
- ⏳ **`http://` 로 들어와도 `https://` 로 자동 전환할까? — 원장 판단 (2026-09-23)**: 지금 GitHub Pages 의 `https_enforced=false` 라 `http://jung-plant.com` 으로 들어오면 **알림 기능 자체가 안 켜진다**(브라우저가 안전하지 않은 주소에서는 서비스워커를 안 띄운다). 켜면 그 문제가 사라진다. **보안 설정이라 전역 규칙대로 손대지 않고 말씀만 드린다** — "켜라" 하시면 켠다.
- ⏳ **`.nojekyll` 을 다시 켤까? — 원장 판단 (2026-09-23)**: Claude 가 Pages 빌드가 실패한 줄 알고 넣었다가, 실제 배포(Actions)는 내내 정상이었음을 확인하고 **되돌렸다**(`b583af2`). 되돌린 이유는 그것 때문에 `/.gitignore`·`/.claude-project.json` 같은 점 파일이 공개됐기 때문(200 → 되돌린 뒤 404). 원장님 "공개한다" 결정은 계좌 일부+잔액·실명에 대한 것이라 Claude 실수로 넓어진 이 노출까지 승인하신 것으로 보지 않았다. **다시 켜길 원하시면 한 줄만 주시면 된다.**
