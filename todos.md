...
**✅ 다음 세션이 가장 먼저 할 일**
0. **(2026-09-23 22:49 원장 답 받음 — 노션 「20260923-2」)** 순서 `ㅇㅋ` · 웹훅 배포 확정 · 계정 주소 안 되살림 · 톡톡 1~3 이미 설정 · 크레딧 항목 뺌 · 결과보고서는 이 작업 끝나고. 로컬에서 할 일 3개: ⓐ 직원허브 통합을 코덱스 담당으로 뻐스디 등록 ⓑ 구현지시서 첫 줄에 "뻐스디 먼저 읽기" 넣고 원장께 넘겨도 된다고 보고 ⓒ `deposit-webhook` 배포(클라우드 세션은 Supabase 함수 목록이 비어 보여 보류). 자세한 건 아래 미답변 해당 줄.
