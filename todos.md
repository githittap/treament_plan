...
## ✅ Task 6 공지 첨부·예치금 권한 분리 — 로컬 검증 완료, 운영 적용 대기 (2026-09-21)
- 구현 기준 `2e1de31`: 공지 첨부는 private `notice-attachments` 버킷과 승인·활성 직원의 자기 UUID 임시 경로를 사용하고, 예치금 조회는 데스크·chief·owner로 분리한다.
- 검증 커밋 `ffb58fb`: 고정 PGlite 0.5.8 합성시험, 정적 수용시험, 관련 서류 회귀시험, 인라인 JS 구문검사와 `git diff --check`를 통과했다.
