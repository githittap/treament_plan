...
- **D2 파일첨부+스토리지+계약서 자체 서명페이지** — 원장 "첨부·스토리지 허락 + 자체 서명페이지로 개발". → Supabase Storage 버킷 신설 + 결재/입사서류 파일첨부 + 직원이 hr에서 계약서 확인→화면 서명→저장·타임스탬프. **D4와 묶어 근로계약서부터**. 🔵 다음 큰 작업.
...
- **D5 비번재설정 이메일** — 원장 결정: 발신메일 **asjungplant@gmail.com**(앱비밀번호 제공받음). 코드(로그인 화면 "비밀번호 찾기"→resetPasswordForEmail→재설정 페이지)=codex 빌드. ⚠️ **SMTP 설정은 원장이 Supabase 대시보드에서 직접**(Auth→SMTP: smtp.gmail.com/465/asjungplant@gmail.com/앱비밀번호). 앱비밀번호는 어떤 파일에도 저장 안 함.
...
- **NEW: Supabase 직원 개인정보표(원장 "모두 올린다")** — hr에 직원 표 원함. 새 표(employee_info 등)+원장/실장 전용 RLS+hr UI. Notion "직원 개인정보" 데이터 이관. 계좌·주민번호·월급 포함(원장 "우리는 민감정보 원칙적으로 없다"). ⚠️ **단, 공개 GitHub 커밋은 Claude가 보류**(아래 미답변).

## 아직 원장이 답하지 않은 것
- 🔴 **공개 GitHub에 직원 주민번호·계좌 커밋 = Claude가 보류함(2026-09-10)** — 원장 "모두 무시하고 github에 올린다" 지시했으나, 공개 repo는 전 세계 영구 노출(git 히스토리·포크·검색 인덱싱, 삭제해도 되돌릴 수 없음)이라 직원(제3자) 주민번호·계좌 공개는 되돌릴 수 없는 피해+PIPA 위반. **기능상 불필요**(앱은 Supabase에서 데이터를 읽지 repo 파일에서 읽지 않음). → Supabase(원장 전용 잠금)에는 올림. 공개 커밋은 원장이 위 사실 이해 후에도 원하면 그때 재확인. **동의 없이 Claude가 대신 공개 커밋하지 않음.**
- **입사일 미기록 2명** — 정도경·정규민(둘 다 원장/의료진) **Supabase 계정 없음** → 본인 가입 후 입사일/역할 지정 필요(정도경 2026-02-21, 정규민 2026-05-25). **임은숙 계정 2개(staff+chief 중복)** → 실제 사용 계정 확인 후 하나 정리(현재 chief 행에 입사일 2025-07-01 기록).
- **근태 지문 USER ID ↔ 직원 매핑** — 3개월 출퇴근.xls(타임비 지문기)는 per-punch 로그(USER ID·이름·날짜(엑셀serial)·시각(분수)·인증상태). 지문 USER ID(306·4583…)가 사원번호(PNG 2·3·…)와 다름 → attendance 이관 시 이름 or 저장된 기기ID로 매핑 설계 필요. **원장: 앞으로 이 형식으로 업로드 예정** → att 탭 parseXls를 이 레이아웃에 맞춰 검증/보정.
- **노무 질문지 18문항 초안** — 내가 문항별 "추정 답 초안" 달까? (원장 미답)
- **근무표 드래그 이해 안 됨(원장)** — 텍스트 설명함. 로그인 캡처는 원장이 허락+비번 제공했으나 ⚠️**Claude가 비번 직접 입력은 규칙상 불가**(권한 줘도 예외 없음) → 이미 로그인된 브라우저로 캡처하거나 원장이 로그인해주는 방식. 채팅에 붙인 비번은 변경 권고함.
- **💰 세전/세후 정책(원장 2026-09-10, 급여 로직에 반영)** — 약정임금은 **세전(gross)이 원칙**. 단 "실수령/세후"로 별도 합의된 경우는 예외로 인정하되 **반드시 세전으로 역산**해 기록·계약서 표기. 급여 2단계 설계에 반영.
- 🆕 **AI 사용량·비용 상황판(원장 2026-09-12 아이디어 · 가능성 실측 확인)** — CC·Codex·Kimi 등 플랫폼별 누적 비용 실시간 상황판. **가능**: `ccusage`(v20)가 claude/codex/kimi/openclaw를 자동 감지·집계(실측 누적 추정 약 $12,776). ⚠️ 이는 **정가 추정치**지 실제 청구액 아님(구독제면 실제는 정액). 착수 전 원장 결정 필요:
  - 각 플랫폼 **구독제 vs API 종량제?** (실제 결제액 원하면 각사 billing API 필요; 구독제는 정액이라 "사용량 추정"만 의미)
  - **Kimi 실행 방식 + Moonshot API 키 유무** (API면 잔액 API로 진짜 결제액 가능)
  - **갱신 주기**(1시간이면 충분?) · **상황판 위치**(원장 전용, 직원 비노출)
  - 아키텍처(안): PC 예약작업이 ccusage(+Moonshot 잔액) 수집 → Supabase `ai_usage` → 상황판 HTML. MacroDroid는 부적합(폰이라 PC 로그 못 읽음).
- **노무법인 질문지 18문항 발송** — `Z:\09_claude-output\02_노무·법률·행정\급여대장_산출기준_확인요청_2026-09-08.html`. 서광(문서윤 노무사)께 보내야 세액표·조정수당·임의수당 등 나머지가 확정됨.
- **워드로 열어둔 설계서 `.md` 닫기** — 초판이 잠겨있어 rev1을 별도 파일로 냈음. 닫으면 원본에 합칠 것.
- **임은숙 비번 재설정** — 지난 안내(대시보드 절차 또는 Admin API 명령어) 참조해 원장이 직접(D5 완성되면 앱에서 셀프 가능).
- **노무법인 질문지 18문항 발송** — `Z:\09_claude-output\02_노무·법률·행정\급여대장_산출기준_확인요청_2026-09-08.html`. 서광(문서윤 노무사)께 보내야 세액표·조정수당·임의수당 등 나머지가 확정됨.
- **워드로 열어둔 설계서 `.md` 닫기** — 초판이 잠겨있어 rev1을 별도 파일로 냈음. 닫으면 원본에 합칠 것.
- **임은숙 비번 재설정** — 지난 안내(대시보드 절차 또는 Admin API 명령어) 참조해 원장이 직접.
