...
- **옛 로그인 계정 2개 영구 삭제** — `asjungplant@asjungplant.com`·`bbc@jungplant.com`. 지우기 전에 public·storage 의 사람 가리키는 uuid 칸 48개를 전부 훑어 참조가 0건임을 확인했다. 이제 **로그인 계정 25개 = 직원 정보 25개**로 1:1이다. 기록: `Z:\09_claude-output\03_병원운영·전산\직원허브_옛계정2개_삭제기록_2026-09-23.md`
...
- ⏳ **프로필 없는 옛 계정 2개 처리(원장 "뭔지 알려줘라")** — `asjungplant@asjungplant.com`(2026-07-06 만들어짐, **한 번도 로그인 안 함**) · `bbc@jungplant.com`(2026-07-06, 마지막 로그인 2026-07-28). 둘 다 직원 정보(프로필)가 없어 지금은 로그인해도 승인 대기 화면만 보인다(접근 불가). 선택: ①그대로 둔다 ②그 계정으로 한 번 로그인하면 승인 대기 목록에 떠서 승인·권한 지정 가능 ③영구 삭제(원장이 "지워라"라고 하면 실행 — 삭제는 승인 필요).
