...
- [ ] 🔴 저장 함수 권한 확인 버그(`consultation_inbox_ingest_service`가 옛 설정값만 읽음, 운영 로그로 확인) — 실행 세션이 패치·되돌리기·시험 작성 중(`fix/ingest-role-check`) → 검수 → 운영 반영 → 네이버 톡톡 send 재시험.
...
- 🔴 **상담문의 저장 함수 버그 발견·수정 중(2026-09-22, Claude 발견)** — `consultation_inbox_ingest_service`가 서비스 권한을 옛 설정값(`request.jwt.claim.role`)으로만 확인하는데 지금 Supabase는 새 방식(`request.jwt.claims`)만 넘겨서 **항상 거부**된다(운영 로그 "service role required"로 확인). 오늘 배포된 기존 상담문의 수집 함수(`consultation-ingest`)도 같은 함수를 써서 같은 문제 — 최근 24시간 호출이 없어 **실제 유실은 없음**. 시험(PGlite)이 옛 설정값을 직접 넣어 통과해 가려졌던 것. 수정: 두 방식을 모두 읽는 패치 SQL·되돌리기·시험을 실행 세션이 작성 중 → Claude 검수 뒤 운영 반영하고 결과를 여기 적는다.
