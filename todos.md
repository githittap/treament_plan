...
- ⏳ **출퇴근 "저녁 추가근무" 칸 — 원장 답: "만든다(직원이 직접 적는 칸)"** → 아직 구현 안 했다. 원장 원본 엑셀 서식에는 있는데 허브에는 야간수당 자동계산만 있다. **2단계 코덱스 몫**으로 검증 세션 `구현지시서.md` 에 넣어야 한다(`Z②-04`).
...
- ⏳ **검증 세션이 찾은 누락 — 원장 판단 필요**: 원장 ZIP 8개의 요구 **131건** 전수 채점 결과 ✅반영 58 · 🟡부분 28 · **❌누락 38** · 🔁변경 4 · ⏸대기 2 · ❌미답 1. 검증 세션 경고 원문: *"T8 수용표는 근무표 ZIP-01~08 8줄뿐이었다 — 나머지 120여 건은 '완료' 보고와 달리 채점된 적이 없다"*. 2단계 구현은 원장이 **코덱스**로 정해 두셨다.
...
- ⏳ **다른 Claude 대화 정리 여부(원장 결정)** — 지금 대기 중인 Claude 대화가 9개, 원격·클라우드까지 하면 더 많다. 정리하고 싶으면 어느 것을 닫을지 알려 주면 확인해 드린다(대화를 닫으면 그 안의 배경 작업도 멈춘다).
- ⏳ **네이버 톡톡 보내기 API 키를 금고에 넣는 법(원장 질문)** — 답: 금고는 파일 하나 `C:\Users\elusi\.secrets\api-keys.env`다. 메모장으로 열어 **맨 아래 새 줄**에 `NAVERTALK_SEND_AUTHORIZATION=붙여넣기`(따옴표·빈칸 없이) 하고 저장한 뒤 채팅에 "금고에 넣었다"만 알려 주면 된다. 값은 채팅·노션에 붙이지 않는다. 지금은 받기만 하므로 없어도 동작하고, 나중에 자동 답장·상담원 전환(핸드오버)을 붙일 때 쓴다.
- ⏳ **프로필 없는 옛 계정 2개 처리(원장 "뭔지 알려줘라")** — 둘 다 2026-07-06에 만들어졌고, 하나는 한 번도 로그인한 적이 없고 다른 하나는 2026-07-28이 마지막 로그인이다(주소는 비공개 기록 참고 — 이 저장소는 공개다). 둘 다 직원 정보(프로필)가 없어 지금은 로그인해도 승인 대기 화면만 보인다(접근 불가). 선택: ①그대로 둔다 ②그 계정으로 한 번 로그인하면 승인 대기 목록에 떠서 승인·권한 지정 가능 ③영구 삭제(원장이 "지워라"라고 하면 실행 — 삭제는 승인 필요).
- ⏳ **"급여 명세서 화면 확인"·"임은숙 실장 재신청 안내"가 무슨 말인지(원장 질문)** — ①급여 명세서: 원장 탭 `💰 급여`(원장 전용 묶음) > 명세서에서 직원·월을 고르면 명세서가 만들어진다. 시급·출퇴근 자료가 아직 0건이라 금액이 비어 보일 수 있고, 모양만 한 번 봐 달라는 뜻이었다. ②임은숙 실장: 예전에 "연차를 신청했는데 안 보인다"고 하셨고 DB에는 신청 기록이 0건이었다. 동시 연차 제한 때문에 저장 자체가 막힌 것으로 보여 빨간 경고 문구를 넣어 배포했으니, **실장에게 한 번 더 신청해 보게 하고 빨간 "⚠ 신청되지 않았습니다"가 뜨는지** 알려 주면 원인을 확정할 수 있다는 뜻이었다.
...
- ⏳ **실제 직원 계정 시험 — 시험 계정 3개 만들기(원장)** — 원장 답: 계정을 만들어 권한을 줄 수 있다. 방법: 실장·매니저·직원 역할 시험 계정 3개(이름 예: 시험-실장, 지메일이면 원장님 지메일 뒤에 +chief 를 붙인 별칭 주소처럼 + 붙인 주소 가능) → hr 회원가입 → 원장 탭에서 승인·권한 지정 → **이 Claude 앱 안의 브라우저 창**에서 원장님이 로그인(비밀번호는 원장님이 입력) → 나머지 시험은 Claude. 시험 뒤 영구삭제 기능으로 정리.
