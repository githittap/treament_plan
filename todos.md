...
- **D5 비번재설정 이메일** — 원장 "착수 가능". 발신메일 **asjungplant@naver.com 또는 @gmail(택1 대기)**. ⚠️ Supabase 커스텀 SMTP 설정 필요(해당 메일 앱비밀번호 발급=원장). 로그인 화면 "비밀번호 찾기"→resetPasswordForEmail→재설정 페이지.
- **D6 통합설명서 + 입사일 입력칸** — 원장 "승인. 단 설명서는 **토큰 아끼게 로컬 유지·추후 온라인배포 리마인드**". 입사일 입력칸(원장탭)은 매니저 완성본 오면 로스터 입력과 함께.

## 아직 원장이 답하지 않은 것
- **직원 개인정보(근태관련.zip = Notion "직원 개인정보" DB, 2026-09-10 수령)** — 재직중 약 15명·퇴사 다수·시작전/대기발령 소수. **입사일 참고용이며 "곧 매니저가 출근 시 완성본 제공" 예정** → 완성본 오면 `profiles.hire_date`·재직여부 정리·입력. ⚠️ 주민번호·계좌·월급 등 고강도 PII → **공개 repo 커밋 절대 금지**(스크래치패드/Z만). 원장이 "직원 간단 표를 hr에도" 원함 → **개인정보 관리 화면 신설 검토**(주민번호/계좌를 Supabase에 저장할지 = 원장 결정 대기).
- **D5 발신 이메일 택1** — asjungplant@naver.com vs @gmail + 그 메일 앱비밀번호(SMTP) 발급.
- **노무 질문지 18문항 초안** — 내가 문항별 "추정 답 초안" 달까? (원장 미답)
- **근무표 드래그 이해 안 됨(원장)** — 설명 필요. 로그인 캡처는 원장이 허락했으나 ⚠️Claude가 비번 직접입력은 보안상 금지 → 원장이 브라우저에서 로그인해주거나 탭 캡처 전달.
...
- **선행조건 4건(데이터 입력, 원장/실장이 해야 함)** — 기능이 아니라 데이터라 Claude가 못 채운다:
  ...
  2. **`profiles.hire_date` 15명 채우기** (전원 비어 있어 연차 자동산정 제안이 안 뜸)
