...
- ✅ **노션 댓글 사진 보기 — 해소(2026-09-22 자정 무렵)**: 원장이 앱 안 브라우저에 노션 로그인 → Claude가 시험 페이지 댓글을 화면으로 열어 붙인 사진(문서 사진)을 또렷이 확인. 로그인은 브라우저에 저장되므로 보통은 다시 할 필요 없음 — 다음에 열 때 로그인 화면이 나오면 그때만 원장이 다시 로그인(앱을 껐다 켜도 남는지는 다음 사용 때 확인해 여기 적는다). 아래는 그 전 기록.
