...
## 아직 원장이 답하지 않은 것
- 🔴 **"계좌연동" MacroDroid 매크로 사고 — 복원 확인 필요(2026-09-14)** — AI비용 웹훅 설정 중 원장이 실수로 **원본 "계좌연동"(입금알림) 매크로를 직접 고쳐서 저장**함(URL이 deposit-webhook→ai-billing-webhook, 헤더토큰도 바뀜). 서버(deposit-webhook 함수 자체)는 안 건드려졌음 확인됨(v12 그대로) — 폰의 매크로 포인터만 잘못됨. 복원값 전달함: URL을 `https://texevhsxttfoqkrucfzl.supabase.co/functions/v1/deposit-webhook`로, 헤더 `X-Webhook-Token`을 `depositWebhook20260731LongSecretToken`으로 되돌리기(Body는 원래도 `{sms_message}`라 안 건드림). **원장이 복원 완료했는지 확인 필요.**
