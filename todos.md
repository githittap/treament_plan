...
| 이전에 Claude 가 임의로 지운 계정 주소 2개 | `todos.md`(지워진 상태) · git 이력에는 남음 | ⏳ 되돌릴지는 아직 말씀 없음 |
...
**✅ 다음 세션이 가장 먼저 할 일**
1. 원장 답 확인 — 아래 미답변 맨 위 「뻐스디 장부 화면 2단계」 추천 순서에 `ㅇㅋ` 인지.
...
- ⏳ **코덱스에 구현지시서를 넘겼는지 — 원장 확인 필요 (2026-09-23)**: 파일은 코덱스가 바로 쓸 수 있게 준비를 끝냈다(`Z:\09_claude-output\03_병원운영·전산\직원허브_T8검증\구현지시서.md` 맨 위 「🚦 코덱스는 여기부터」). 코덱스 세션에 줄 한 줄: `Z:\09_claude-output\03_병원운영·전산\직원허브_T8검증\구현지시서.md 읽고 시행하라`. **원장이 실제로 넘기셨는지 알려 주시면 이 줄을 닫는다.**
...
- ⏳ **톡톡 설정창 사진 보고 만들 기능 고르기(원장 "필요한기능이있으면 더 제안해달라")** — 사진 확인했다(톡톡 파트너센터 > 연동 관리 > 스마트톡톡 설정). 제안 목록은 아래 절. 원장이 번호로 고르면 구현한다. **(부분 답 2026-09-23: 4번 알림API는 원장 "덴트웹에서 가능하니 필요없다"로 철회, 5번 옛 상담 135건은 "오늘부터로 한다". 남은 것은 1~3번 — 원장이 파트너센터에서 직접 하는 채팅방 고정 버튼·시간 외 자동응답·첫 인사말 — 선택 사항)**
