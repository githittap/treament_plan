...
- [ ] **메뉴 조직도 실제 반영 진행 중**(Opus 실행 세션, `feat/menu-restructure`): 원장 확정 구조 + 정정 2건(근무 묶음을 원장 전용 밖으로, 커뮤니티는 비워 둠)을 hr.html 두 단계 메뉴와 도구 모음 `index.html` 묶음에 반영, 홈 카드 3개 제거, 라벨 변경, 권한 불변. "치료계획 도구를 직원허브 탭으로"(hub 메모)는 범위 밖 다음 후보.
