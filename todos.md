...
- ⏳ **새 신호어 만들 것 고르기 — 원장 선택 (2026-09-24, 노션 「0924-허브채점()」 댓글 요청)**: 원장 원문 *"페이블_필드가이드_나의_언노운_찾기_v1.0.html ← 이것을 참조하여 신호어중에 더 만들것을 추천해줘."* 추천(우선순위): ①`#언노운`(블라인드스팟 패스 — 새 일 시작 전 내가 모르는 것·물어볼 것·함정부터. Claude엔 「언노운」 말로 도는 확신도 스킬이 이미 있어 정식 신호어로 묶고 코덱스까지 맞춤) ②`#인터뷰`(역질문 — AI가 한 번에 하나씩, 결과가 갈리는 것부터 원장에게 묻기) ③`#계획서`(실행계획 — 바로 만들지 말고 계획서부터, 원장이 볼 결정은 맨 위) ④`#퀴즈`(리뷰퀴즈 — 원장 이름으로 나가는 고위험 문서는 요점 보고+3~5문제, 다 맞히기 전 발행 보류). 보류 권함: 레퍼(「이거 참고해서」로 충분) · 실행기록(원장 신호보다 AI가 늘 하는 규칙) · 결과보고(결과보고서 자체가 보류 중) · 여러 방향 시안(기존 #시안과 겹침). **원장 답 형식**: 만들 번호(예: 「1,3」) 또는 `ㅇㅋ`(넷 다). 답 오면 SIGNAL-WORDS.md 3장 10단계대로 만든다.
