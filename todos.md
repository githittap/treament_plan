...
## 아직 원장이 답하지 않은 것
- 🆕 **통합설명서 계획 답변 대기(2026-09-09, 실행 안 함·계획만)** — 아래 D1~D6은 원장이 결정해야 착수. 상세는 통합설명서 「6. 결정 대기」:
  - **D1 '비밀 진료기록' 개명** — 어감·분쟁신고 우려로 개명 요청. 후보: 케이스 노트/특이사항 노트/제한열람 메모. 라벨만 바꾸면 됨(작음). → 이름 택1 대기.
  - **D2 근로계약서 근로자 서명칸 + 결재 파일첨부** — 현재 파일 저장소(Storage) 0개, 결재는 텍스트+도장만. approval_docs.attachments 컬럼만 준비됨. 서명 방식 A(자체페이지·추천)/B(외부 전자서명)/C(종이) 택1 대기.
  - **D3 jung-plant.com 허브 정리** — index.html 9카드 평면 나열(임상·직원·재미·정보 혼재, 로그인도구 노출). 4구역 그룹+로그인 게이트 계획. 착수 시점 대기.
  - **D4 도장·컬러 서식 자동생성** — 개인도장(stamp_html.py)·병원직인 스킬·컬러#156f72 있음. 근로계약서 등 서식 자동생성. 어떤 서식부터 대기.
  - **D5 비번 재설정 이메일 앱 탑재** — 원장 "넣는 게 좋다"(동의). Supabase resetPasswordForEmail. ⚠️전제: 직원 계정 이메일 등록 현황 확인 필요. 착수 대기.
  - **D6 앱 내 '도움말' 버튼 + 입사일 입력칸** — hire_date는 원장탭에 표시만 되고 편집칸 없음(직접 DB입력 필요). 앱내 도움말 만들지 대기.
  - **노무 질문지 18문항 초안** — 내가 각 문항에 "추정 답 초안"을 달아 노무사에게 확인만 받게 할까? 대기.
  - **스크린샷 방식** — 로그인 필요해 Claude가 실제 화면 캡처 불가. 원장이 각 탭 캡처해 주시면(방법 A) 내가 화살표·설명 얹어 설명서에 삽입.
- **비밀번호 재설정 이메일 기능** — (D5로 승격, 원장 "넣는 게 좋다" 표명) 착수 여부·이메일 등록 현황 확인 대기. 지금은 Supabase 대시보드에서 원장이 직접 처리.
