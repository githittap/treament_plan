...
- 🆕 **AI비용 원장전용 탭(2026-09-14, 원장 지시 "실제청구액 기준")** — hr에 owner 전용 `💰 AI비용` 탭 codex 빌드중: 월별 플랫폼 실제 청구액을 **원장이 직접 입력**(`ai_billing` 표, owner RLS)·총액 표시. 로컬 ccusage 상황판(정가 환산 사용가치)은 참고용으로 별도 유지. "직원허브에서 바로가기"=owner 탭으로 접근.
