...
- 🆕 **AI 사용량·비용 상황판(원장 확정 2026-09-12)** — **원장 전용**. **₩(당일 환율 환산) 표기**, **6시간 주기**. 수집: ccusage(claude/codex/kimi) + Moonshot 잔액 API(Kimi 실결제) → Supabase `ai_usage` → 상황판 HTML. 플랫폼 실측 분류: Claude=Max20x 구독+**초과 크레딧 실결제($521/$800)**, Kimi=Moonshot API 종량, 세번째(시트+크레딧 자동충전 ~$28)=제품명 원장 확인要(OpenAI/Codex 추정). MacroDroid 부적합.
...
- **임은숙 계정 2개 정리 — 조치안 확인 요청** — 최근 로그인=gmail(eunsooki4128, **staff**, 9/10 23:21)·기존=daum(les4128, **chief/실장**, 7/31 생성). 원장 기준(최근=실사용)이면 gmail이 현행이나 **실장 권한은 daum에 있음**. 권장: gmail을 chief로 승격 + daum 비활성(⚠️하드삭제 금지 — 연차·기록 FK). 실행 전 확인: 두 계정에 연차/기록이 나뉘어 있을 수 있음.
...
- **노무 질문지 발송(초안 완료)** — ✅ 문항별 추정답 초안 작성·전달: `Z:\09_claude-output\02_노무·법률·행정\급여대장_산출기준_확인요청_답변초안_2026-09-12.html`. 원장 검토 후 서광(문서윤 노무사) 발송만 남음.
