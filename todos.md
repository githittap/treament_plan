...
- ✅ **AI비용 원장전용 탭 배포(2026-09-14)** — hr에 owner 전용 `💰 AI비용` 탭(직원허브 안에서 접근). 월별×플랫폼(Claude/Codex/Kimi/기타) **실제 청구액을 원장이 직접 입력**(`ai_billing` upsert)·이번달 총액 큰 글씨·최근6개월 막대이력. 로컬 ccusage 상황판(정가 환산 사용가치)은 참고용 별도 유지. ⏳ 원장 로그인 후 값 한 번 입력해 확인 필요.
