...
- [ ] 실행 세션 진행 중: 실제 푸시 발송(코드·시험, 운영 반영 전) · 계정 영구삭제(근무 기록 보존형, 코드·시험, 운영 반영 전) · 급여 2단계 첫 조각(명세서, 시험용 숫자) · KB·표준·인박스 기록 갱신.
...
## 아직 원장이 답하지 않은 것
- ⏳ **외부 상담 연결 — 방식 결정·가입(2026-09-22 조사, 원장 결정 필요)** — 네이버 톡톡: 무료 공식 연동 가능(톡톡 파트너센터 가입 → 챗봇 API 신청, 심사 1~2일) → 원장 가입 필요. 카카오: ① 무료 자동응답(오픈빌더, 첫 응대가 자동응답이 됨) ② 유료 상담톡(공식 딜러 계약, 사람 상담 그대로 연동) 중 선택. 당근: 공식 연동 없음 → 앱에서 직접 확인 유지(원하시면 당근 비즈니스 고객센터에 제휴 API 문의). 알림톡은 보내기 전용이라 해당 없음. 매크로드로이드 알림 수집은 설계상 PC 안에만 두는 도구라 자동 연결하지 않는다. 출처: 조사 보고(공식 문서 링크 포함)는 이 세션 기록.
- ⏳ **실제 직원 계정 시험(4역할) — 시험 계정 준비(원장)** — 실장·매니저·직원 역할의 시험용 계정으로 원장님이 앱 안 브라우저에서 로그인해 주시면 나머지(저장·재조회·권한 확인)는 Claude가 진행한다(Claude는 계정을 만들거나 비밀번호를 입력할 수 없음). 실제 직원 계정을 쓸지, 시험용 가입을 새로 할지 결정 필요.
- ⏳ **jung-plant.com 허브 정리 — '정리'가 무엇인지(원장)** — 지금은 도구 11개가 한 화면에 섞여 있다(진료 도구 + 사주·직원뽑기 같은 재미 도구). 재미 도구를 따로 모을지, 원장 전용 링크를 숨길지, 디자인만 바꿀지 알려 주시면 바로 진행.
...
- ⏳ **결과보고서를 언제 만들까요?(Codex 규칙과 충돌, 원장 결정)** — Claude 규칙(RESULT-REPORT.md)은 "작업이 끝나면 항상 결과보고서", Codex 규칙(9/21 원장 지시)은 "원장이 요청할 때만(복잡한 작업 뒤 '결과보고서 만들까요?' 한 줄 제안)". 둘 중 하나로 맞출지, 지금처럼 따로 둘지 알려 주시면 규칙을 고친다. 결정 전까지 Claude는 기존 규칙(항상)을 따른다.
