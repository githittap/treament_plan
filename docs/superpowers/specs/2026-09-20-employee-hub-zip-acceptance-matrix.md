...
| B. 후속 `허브관련 2차.zip` | 요구 정리·후속 작업 대기 | 구현 착수 전 요구와 수용 기준을 분리 확인 |
| C. `직원허브 3차.zip` 피드백 | 초안/결정 대기 | 확정되지 않은 문구·동작을 구현 사실로 기록하지 않음 |
| D. 4차 UI 및 캘린더 보완 | 이 worktree에서 통합되어 `0f3226b`까지 배포 | 실제 로그인·역할별 운영 조작은 미검증 |
| 후속 상담일지 요구 | 기본 migration 운영 적용, RLS 8/8 PASS·Sol PASS | Advisor 보완 SQL의 운영 적용과 실제 로그인 시험은 별도 승인 |

- 현재 main: `0f3226b` (2026-09-21 직접 push 완료).
