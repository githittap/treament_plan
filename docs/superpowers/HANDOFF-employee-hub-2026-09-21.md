...
- 현재 실행: Terra 단일 실행 작업 `01a0c0ca-8508-7273-bed7-ef9cb82e872b`; Sol 읽기전용 검증 `01a0c065-5575-7b91-8a68-a048e19f1828`.
- 감독: heartbeat `zip` 10분. Kimi/BUSD 경로는 취소 상태다.

## 아직 미완료 — Task 6~11

| Task | 다음 작업 | 외부 조건/승인 게이트 |
|---|---|---|
| 6 | 공지 첨부와 예치금 접근의 남은 요구를 최신 원문 기준으로 확정·구현 | Storage 버킷/RLS와 기존 자료 보존 영향을 검토하고, 승인된 최초 ZIP 범위 안에서 단계배포 |
...
2. Task 6~11은 위 표 순서대로, 각 원문 요구와 수용 기준을 먼저 확정한 뒤 최소 변경으로 진행한다. Task 11 완료 뒤 `C:\Users\elusi\Downloads\상담문의 등 일원화.zip`을 별도 Sol High 작업으로 전달하고, 결과보고서·사용설명서를 갱신한 뒤 heartbeat 감독을 종료한다.
