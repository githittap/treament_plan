...
| E. Task 6 공지 첨부·예치금 | 로컬 구현·합성검증 완료, 운영 적용·배포 미수행 | `2e1de31` + `ffb58fb`; private Storage/RLS 초안은 별도 적용 검토가 필요하다. |
...
- Task 6: `NOTICE_ATTACHMENTS_ACCEPTANCE_PASS`, `PGLITE_NOTICE_ATTACHMENTS_DEPOSIT_ACCESS_PASS`, 인라인 JS 구문검사, `git diff --check`를 로컬에서 통과했다. 이 증거는 운영 DB/Storage 적용이나 실제 역할 로그인 시험을 대신하지 않는다.
...
|---|---|---|
| 6 | 공지 첨부와 예치금 접근 | 로컬 구현·PGlite 합성검증 완료. 운영 Storage 버킷/RLS 적용, 별도 시험 계정의 실제 업로드·저장/재조회, push·배포는 미수행 |
