...
- 갱신: 2026-09-21 09:43 KST
- 현재 배포 정본: `origin/main` = `0f3226b` (직접 push 완료)
- 이 인수인계의 로컬 기록 커밋은 아직 `origin/main`에 포함하지 않는다.
...
|---|---|---|
| 6 | 공지 첨부와 예치금 접근의 남은 요구를 최신 원문 기준으로 확정·구현 | Storage 버킷/RLS와 기존 자료 보존 영향 검토, 운영 적용 별도 승인 |
...
| 11 | 결과보고서·사용설명서 | Task 6~10의 객관적 시험 결과를 반영해 최신화 |

## 운영 시험 경계와 다음 순서

1. 별도 시험 계정으로 manager·owner·staff·chief 각각의 로그인/조회/입력/수정/저장 후 재조회 경계를 시험한다. 실제 환자·직원 원본은 사용하지 않는다.
2. Task 6~11은 위 표 순서대로, 각 원문 요구와 수용 기준을 먼저 확정한 뒤 최소 변경으로 진행한다.
